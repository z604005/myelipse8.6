...
public class EX010114R0F extends BA_EXCEL {
	 ...
	 * 
	 */
	public EX010114R0F(Connection conn,String rpt,String SC0030_01) throws Exception{
		
		//樣版檔的名稱，注意此元件只能處理樣板上只有一個sheet
		super.TemplateName = "EHF010114R0.xls";
		
		//sheet的名稱
		super.SheetName = "員工離職證明書";
...
//		super.PageColCount = 10;
	 ...
	 * 
	 */
	public EX010114R0F(Connection conn,String rpt,String SC0030_01,HttpServletRequest request) throws Exception{
...
//		設定request
	...
	*/
	...
	public int print(Connection conn, EHF010114M0F Form, String comp_id) {
		// TODO Auto-generated method stub
		...
		try{
			...
			" IFNULL(C.EHF010106T1_01,''), IFNULL(DATE_FORMAT(C.EHF010106T1_02, '%Y/%m/%d'),'') AS EHF010106T1_02_take, " +
			" IFNULL(D.EHF010106T1_01,''), IFNULL(DATE_FORMAT(D.EHF010106T1_02, '%Y/%m/%d'),'') AS EHF010106T1_02_leave, " +
			...
			" LEFT JOIN EHF010106T1 C ON A.HR_EmployeeSysNo = C.HR_EmployeeSysNo AND A.HR_CompanySysNo = C.HR_CompanySysNo AND C.EHF010106T1_01 = '1' " +
			" LEFT JOIN EHF010106T1 D ON A.HR_EmployeeSysNo = D.HR_EmployeeSysNo AND A.HR_CompanySysNo = D.HR_CompanySysNo AND D.EHF010106T1_01 = '2' " +
			...
			while(rs.next()) {
				...
				EHF010114M0F bean = new EHF010114M0F();
				...
				bean.setEHF010106T1_02_take(rs.getString("EHF010106T1_02_take"));  //
				bean.setEHF010106T1_02_leave(rs.getString("EHF010106T1_02_leave"));  //
	...
	private void printDetail(List dataList, Map queryMap, int displacement) {
		...
		try {
			...
			while(it.hasNext()){
				...
				//建立新頁面
				this.newPage();								
				EHF010114M0F bean = (EHF010114M0F)it.next();
				...
				this.setValue(6+displacement, "E", bean.getEHF010106T0_10(), false);
				this.setValue(7+displacement, "B", bean.getEHF010106T1_02_take(), false);
				this.setValue(7+displacement, "E", bean.getEHF010106T1_02_leave(), false);
