...
/**
 * 列印EXCEL用的元件(產品的薪津清冊專用)
 * @version 1.6
 * @created 29-八月-2013 上午 09:20:12 整理
 */
public class BA_EXCEL_forsalary {
	...
	protected Alignment Alignment=jxl.format.Alignment.CENTRE;//把水平對齊方式指定為居中 
	protected VerticalAlignment VerticalAlignment=jxl.format.VerticalAlignment.CENTRE;//把垂直對齊方式指定為居中 
			...
			if (!HeadData[2][i-1].trim().equals("")){
				if (HeadData[4][i-1].trim().equals("")){
					if (HeadData[3][i-1].trim().equals("Y")){
						...
						if(Integer.parseInt(HeadData[0][i-1])==1&&"A".equals(HeadData[1][i-1])){
							this.createFont=WritableFont.createFont("新細明體");
							this.size=20;
						}else if(Integer.parseInt(HeadData[0][i-1])==2&&"A".equals(HeadData[1][i-1])){
							this.createFont=WritableFont.createFont("新細明體");
							this.size=16;
						}else {
							this.createFont=WritableFont.createFont("新細明體");
...
//		存放標題用的陣列
	 ...
	 * ScolName 要複製樣板的欄位名稱
	 */
	public void CopyCellFormat(int Drow,String DcolName,int Srow,String ScolName,boolean change_Size) throws Exception{
		int Scol = trueCol(ScolName)+1;
		int Dcol = trueCol(DcolName)+1;
		CopyCellFormat(Drow,Dcol,Srow,Scol,change_Size);
	}
	 ...
	 * Scol 要複製樣板的欄位 (從1開始)
	 */
	public void CopyCellFormat(int Drow,int Dcol,int Srow,int Scol,boolean change_Size) throws Exception{
		int scol = Scol-1 ;
		int dcol = Dcol-1;
		...
		if(change_Size)
			sheet.setRowView(Drow-1+row,1300);
	}
...
//		丟頁計
	...
	private void writeValue(int Trow,int DataCol,String Sdata,boolean PrintZero) throws Exception{
		...
		WritableFont wfc = new WritableFont(createFont, size);//設定字體與大小
		// =======================為了薪津清冊而寫的==========================================================================================
		...
		if(Trow==1&&DataCol==0){
			...
			wfc.setBoldStyle(WritableFont.NO_BOLD);  
		}
		...
		if (!Sdata.equals("")) {
			...
			wcf.setAlignment(Alignment); // 對齊方式
			wcf.setFont(wfc);
			wcf.setVerticalAlignment(this.VerticalAlignment);  
			if(Trow>=5&&DataCol>=2){
				//框線
	...
	protected int trueRow(int Vrow) throws Exception{
		...
		return (RecordStart-1)+((1-1) * RecordRow)+(Vrow-1)+1;
	}
	/** 
	 * 寫入資料內容部份
	 * Vrow 一筆資料也許有兩列以上，所要丟的資料在第幾列(通常為1)
	...
	public void mergeCell(int rowTopLeft,String colTopLeftName,int rowBottomRight,String colBottomRightName ,String aa) throws Exception{
		//合併儲存格
		int Lrow = (rowTopLeft -1)+((PageCout-1) * PageRowCount);
		int Rrow = (rowBottomRight -1)+((PageCout-1) * PageRowCount);		
		WritableFont myFont = new WritableFont(WritableFont.createFont("新細明體"),12);
		...
		Label d7_to_g7 = new Label(trueCol(colTopLeftName), Lrow,aa, cellFormat);  
