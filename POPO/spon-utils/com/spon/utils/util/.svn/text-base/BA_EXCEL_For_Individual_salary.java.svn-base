...
/**
 * 列印EXCEL用的元件(產品的個人薪資條專用專用)
 * @version 1.6
 * @created 29-八月-2013 上午 09:20:12 整理
 */
public class BA_EXCEL_For_Individual_salary {
...
//		丟頁計
	 ...
	 *丟下一筆資料 (資料少於25筆時)
	 */
	public void next_Small() throws Exception{
		if (rowCount==0){ //第一次進入
//			設定頁面格式
			newPage_Small();			
		}else{
...
//				設定頁面格式
	 ...
	 * 換頁 (資料少於25筆時)
	 */
	protected void newPage_Small() throws Exception{
		CellFormat ss = null ;
		Label labelCF = null;
		Blank blank = null;
		//丟上一頁的頁計
//		丟頁計
		...
		for(int j=0;j<PageRowCount;j++){ //列數
			switch(j){
			case 0:	
				sheet.setRowView(j+Row,500);
				break;
			case 1:	
				sheet.setRowView(j+Row,342);
				break;
			case 2:	
				sheet.setRowView(j+Row,330);
				break;
			case 27:	
				sheet.setRowView(j+Row,500);
				break;
...
//		取得位置
	...
	private void writeValue(int Trow,int DataCol,String Sdata,boolean PrintZero) throws Exception{
		...
		if (!Sdata.equals("")) {
			// 設定格式
			//System.out.println(cf.getBackgroundColour());
			wcf = new WritableCellFormat(cf);
			wcf.setBackground(colour);// 增加顏色
			wcf.setAlignment(this.Alignment); // 對齊方式
			wcf.setFont(wfc);
	...
	public void mergeCell(int rowTopLeft,String colTopLeftName,int rowBottomRight,String colBottomRightName ,String aa, int type) throws Exception{
		//合併儲存格
		int Lrow = (rowTopLeft -1)+((PageCout-1) * PageRowCount);
		int Rrow = (rowBottomRight -1)+((PageCout-1) * PageRowCount);		
		WritableFont myFont = new WritableFont(createFont,size);
		switch(type){
