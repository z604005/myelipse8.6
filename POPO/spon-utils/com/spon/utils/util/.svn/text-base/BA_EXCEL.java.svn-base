...
public class BA_EXCEL {
	...
	protected Alignment Alignment_RIGHT=jxl.format.Alignment.RIGHT;//把水平對齊方式指定為靠右
	...
	protected VerticalAlignment VerticalAlignment_BOTTOM=jxl.format.VerticalAlignment.BOTTOM;//把垂直對齊方式指定為靠下對齊
...
//		下面的都不要改
        ...
        //sheet.getSettings().setPassword("123") ;	// 設置保護密碼
	}
...
//             lbl.setCellFormat(cell.getCellFormat());
	...
	/**
	 * 合併儲存格
	 * @param rowTopLeft 要合併儲存格的左上方列數 (從1開始)
	 * @param colTopLeftName 要合併儲存格的左上Col如 "A" or "AB"
	 * @param rowBottomRight 要合併儲存格的右下方列數 (從1開始)
	 * @param colBottomRightName 要合併儲存格右下的Col如 "A" or "AB"
	 * @param Alignment 水平對齊方式，LEFT、RIGHT、CENTRE
	 * @param VerticalAlignment 垂直對齊方式，TOP、CENTRE、DOWN
	 * @param Wrap 換行，Y：true，N：false
	 * @param ShrinkToFit 縮小字型已適合欄寬，Y：true，N：false
	 * @param character 資料內容
	 * @param TOP_Border 上框線 TOP
	 * @param LEFT_Border 左框線 LEFT
	 * @param RIGHT_Border 右框線 RIGHT
	 * @param BOTTOM_Border 下框線 BOTTOM
	 * @param 當框線參數均為空白("")時預設4邊全部都有框線
	 * @param cellHight 設定列高，如492約略為25，零(0)表示預設不進行設定
	 * @param cellwidth 設定欄寬
	 * @param Font 內文字體，空白("")時預設"新細明體"
	 * @param FontSize 內文字體大小，零(0)時預設12
	 * @param BOLD 粗體，Y或N
	 * @throws Exception
	 * 修改於20150428 BY Troy
	 */
	public void mergeCells(int rowTopLeft,String colTopLeftName,int rowBottomRight,String colBottomRightName,String Alignment,String VerticalAlignment,
						   String Wrap, String ShrinkToFit, String character, String TOP_Border, String LEFT_Border, String RIGHT_Border, 
						   String BOTTOM_Border, int cellHight, int cellwidth, String Font, int FontSize, String BOLD) throws Exception{
		
		//合併儲存格
		int Lrow = (rowTopLeft -1)+((PageCout-1) * PageRowCount);
		int Rrow = (rowBottomRight -1)+((PageCout-1) * PageRowCount);
		
		if("".equals(Font)&&Font==null){	//預設
			Font = "新細明體";
		}
		if(FontSize==0){
			FontSize = 12;
		...
		WritableFont myFont = new WritableFont(WritableFont.createFont(Font),FontSize);
		...
		if("LEFT".equals(Alignment)){
			...
			//把水平對齊方式指定為靠右
			cellFormat.setAlignment(Alignment_RIGHT);
		...
		}else if("CENTRE".equals(VerticalAlignment)){
			//把垂直對齊方式指定為居中 
			cellFormat.setVerticalAlignment(VerticalAlignment_CENTRE);
		}else if("BOTTOM".equals(VerticalAlignment)){
			//把垂直對齊方式指定為靠下對齊
			cellFormat.setVerticalAlignment(VerticalAlignment_BOTTOM);
		}else{
			//把垂直對齊方式指定為居中 
			cellFormat.setVerticalAlignment(VerticalAlignment_CENTRE);
		}
		
		if("TOP".equals(TOP_Border)){
			cellFormat.setBorder(Border.TOP, BorderLine, Colour.BLACK);
		}
		if("LEFT".equals(LEFT_Border)){
			cellFormat.setBorder(Border.LEFT, BorderLine, Colour.BLACK);
		}
		if("RIGHT".equals(RIGHT_Border)){
			cellFormat.setBorder(Border.RIGHT, BorderLine, Colour.BLACK);
		}
		if("BOTTOM".equals(BOTTOM_Border)){
			cellFormat.setBorder(Border.ALL, BorderLine, Colour.BLACK);
		}
		if("".equals(TOP_Border)&&"".equals(LEFT_Border)&&"".equals(RIGHT_Border)&&"".equals(BOTTOM_Border)){
			cellFormat.setBorder(Border.ALL, BorderLine, Colour.BLACK);
		}
		
		sheet.mergeCells(trueCol(colTopLeftName),Lrow,trueCol(colBottomRightName),Rrow);
		Label labelCF = new Label(trueCol(colTopLeftName), Lrow, character, cellFormat); 
		sheet.addCell(labelCF);
		
		if(cellHight!=0){
			if(rowTopLeft==rowBottomRight){
				sheet.setRowView(Rrow,cellHight);//設定列高
			}else{
				sheet.setRowView(Lrow,cellHight);//設定列高為cellHight
				sheet.setRowView(Rrow,cellHight);//設定列高為cellHight
			}
		}
		if(cellwidth!=0){
			if(colTopLeftName.equals(colBottomRightName)){
				sheet.setColumnView(trueCol(colTopLeftName), cellwidth);//設定欄寬
			}else{
				sheet.setColumnView(trueCol(colTopLeftName), cellwidth);//設定欄寬為cellwidth
				sheet.setColumnView(trueCol(colBottomRightName), cellwidth);//設定欄寬為cellwidth
			}
		}
	...
	/**
	 * 列印圖片
	 * @param StartX 水平軸起始欄位，由0開始，例:0.5表示起始位置為第0.5格水平欄位，由左至右
	 * @param StartY 垂直軸起始欄位，由0開始
	 * @param picWidth 圖片占用水平軸欄位數，例:2.5表示占用2.5個水平欄位
	 * @param picHeight 圖片占用垂直軸欄位數
	 * @param StartShift 水平軸與垂直軸的偏移量，避免圖片將欄位線條蓋掉
	 * @param picShiftX 圖片占用水平軸欄位數的偏移量，避免圖片將欄位線條蓋掉
	 * @param picShiftY 圖片占用垂直軸欄位數的偏移量，避免圖片將欄位線條蓋掉
	 * @param TopToDown 垂直軸的起始，1：由上到下，2：由下到上
	 * @throws Exception
	 * 修改於20150427 BY Troy
	 */
	public void print_Picture(double StartX, double StartY, double picWidth, double picHeight, 
							  double StartShift, double picShiftX, double picShiftY, String TopToDown)throws Exception{
		//jxl插入圖片
		...
		if("1".equals(TopToDown)){	//垂直軸由上到下數回來
			sheet.addImage(new WritableImage(StartX+StartShift, StartY+StartShift, picWidth-picShiftX, picHeight-picShiftY, baos.toByteArray()));
		}else if("2".equals(TopToDown)){	//垂直軸由下到上倒數回來
			sheet.addImage(new WritableImage(StartX+StartShift, sheet.getRows()-(StartY-StartShift), picWidth-picShiftX, picHeight-picShiftY, baos.toByteArray()));
		}
	}
	 ...
	 * @param picShiftY 圖片占用垂直軸欄位數的偏移量，避免圖片將欄位線條蓋掉
	 * @param TopToDown 垂直軸的起始，1：由上到下，2：由下到上
	 * @param result 電子簽名輸入串流資訊
	 * @throws Exception
	 * 修改於20150427 BY Troy
	 */
	public void print_jSignPicture(double StartX, double StartY, double picWidth, double picHeight, 
			  double StartShift, double picShiftX, double picShiftY, String TopToDown, ByteArrayInputStream result)throws Exception{
		//jxl插入圖片
		//1. 圖片必須是png檔，所以取得圖片後，不論是bmp, jpg, png, gif等...皆轉成png檔
		//java.io.File imageFile = new java.io.File(getServlet().getServletContext().getRealPath("") + "/pic/2011060005.png");
		BufferedImage input = ImageIO.read(result);
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		ImageIO.write(input, "PNG", baos);
		double width = 1, height = 1;
		double x_shift = 0, y_shift = 0;
